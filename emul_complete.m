...
% to the rows of M_Pc is output, according to whether 'cov' or 'var' is
% specified as last input.
% The variance from the components not used in the linear combination is 
% added to the variance arising from the linear combination.
%
% INPUTS:
% M_Pc:   Txn matrix (n>=r). In j-th column, emulated mean of the
%         coefficient of the j-th PC.
% Cov_Pc: Usually, TxTxr. Cov(:,:,j) represents the covariance matrix of
%         the coefficient of the j-th PC. (last dimension can be >r)
%         If input var_cov is the string 'var', then Cov_Pc should be a Txr
%         matrix, with only emulated variances in each column.
% PC_Val: Nloc x n matrix. In each row, values of the n PCs at the location
%         corresponding to that row. There are Nloc locations.
% Mn_Val: Nloc x 1, with average value of the original simulation outputs
%         at each of the N locations. This will be added to the linear
%         combination of the columns of Pc_Val.
% Std_PCA: Row or column vector of length n, with standard deviation of PCs
%         (from eigenvalue decomposition).
% r:      number of PCs to use in linear combinations
% var_cov: a string, either 'var' or 'cov'. Accordingly, only variance or
%          full covariance will be computed as output Cov.
%
%
% OUTPUTS:
% M: T x Nloc, with emulated mean temperature at time i and location j
%    stored in M(i,j).
%
% Second output changes according to whether 'var' or 'cov' is provided as
% var_cov input.
%
% If 'var', then:
